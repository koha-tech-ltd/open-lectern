...
export function SiteChrome({
  ...
  return (
    <div className="grain flex min-h-screen min-w-0 flex-col text-ink">
      <header className="relative z-40 border-b border-walnut/10 bg-ivory/70 backdrop-blur-sm">
        <div className="site-chrome-bar mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:gap-4 sm:px-6 sm:py-4">
          <div className="site-chrome-cluster flex min-w-0 flex-wrap items-center gap-3 sm:gap-6">
            <Link to="/" className="site-chrome-brand flex items-center gap-3 no-underline">
              <img
                src="/logo.png"
                alt="Lectern"
                className="site-chrome-logo h-12 w-12 rounded-md object-cover shadow-lectern"
              />
              <div>
          ...
          <div className="site-chrome-tools flex flex-wrap items-center gap-3">
            {onHowItWorks ? (
              <button
                type="button"
                className="site-chrome-how text-sm text-forest underline-offset-2 hover:underline"
                onClick={onHowItWorks}
              >
                {t('chrome.howThisWorks')}
              </button>
            ) : null}
            <div className="site-chrome-lang" data-lectern-target="language">
              <LanguageSelector />
            </div>
          </div>
        </div>
      </header>
      <main className="site-chrome-main mx-auto flex w-full min-w-0 max-w-6xl flex-1 flex-col px-4 py-6 sm:px-6 sm:py-10">
        {children}
      </main>
